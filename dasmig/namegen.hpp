...
class ng
{
  public:
    ...
    class name
    {
      public:
        ...
        name append_name()
        {
            _internal_string.append(L" ").append(
                ng::instance().get_name(_gender, _culture));
            return *this;
        }
        ...
        {
            _internal_string.append(L" ").append(
                ng::instance().get_name(_gender, culture));
            return *this;
        }
        ...
        {
            _internal_string.append(L" ").append(
                ng::instance().get_surname(_culture));
            return *this;
        }
        ...
        {
            _internal_string.append(L" ").append(
                ng::instance().get_surname(culture));
            return *this;
        }
        ...
        friend std::wostream& operator<<(std::wostream& os, const name& name)
        {
            os << name._internal_string;
            return os;
        }
      ...
      private:
        ...
        name(const std::wstring& name_str, gender gender, culture culture)
            : _internal_string(name_str), _gender(gender), _culture(culture)
        {
        }
    ...
    {
        return (_country_code_map.find(country_code) != _country_code_map.end())
                   ? _country_code_map.at(country_code)
                   : culture::any;
    }

    // Generates a first name based on requested gender and culture.
    name get_name(gender gender = gender::any,
    ...
    name get_surname(culture culture = culture::any) const
    {
  ...
  private:
    // Typedef to avoid type horror when defining a pointer to a container of
    // names.
    ...
    name solver(bool is_name, gender requested_gender,
                culture requested_culture) const
    {
        ...
        std::uniform_int_distribution<std::size_t> culture_range(
            0, static_cast<std::size_t>(culture::any) - 1);

        // Distribution of possible gender values.
        std::uniform_int_distribution<std::size_t> gender_range(
        ...
        {
            requested_culture =
                static_cast<culture>(culture_range(random_device));
        }
        ...
        if (is_name)
        {
            ...
            case gender::m: {
                ...
                std::uniform_int_distribution<std::size_t> names_range(
                    0,
                    _culture_indexed_m_names.at(requested_culture)->size() - 1);
                // Randomly select a name of the requested gender and culture.
                return name(_culture_indexed_m_names.at(requested_culture)
                                ->at(names_range(random_device)),
            ...
            case gender::f: {
                ...
                std::uniform_int_distribution<std::size_t> names_range(
                    0,
                    _culture_indexed_f_names.at(requested_culture)->size() - 1);
                // Randomly select a name of the requested gender and culture.
                return name(_culture_indexed_f_names.at(requested_culture)
                                ->at(names_range(random_device)),
                            requested_gender, requested_culture);
            }
            default: {
        ...
        else
        {
            ...
            std::uniform_int_distribution<std::size_t> surnames_range(
                0, _culture_indexed_surnames.at(requested_culture)->size() - 1);
            // Randomly select a surname of the requested culture.
            return name(_culture_indexed_surnames.at(requested_culture)
                            ->at(surnames_range(random_device)),
                        requested_gender, requested_culture);
        }
    };
    ...
    void parse_file(const std::filesystem::path& file)
    {
        ...
        // <m(ale)|f(emale)|s(urname)>, list of names.
